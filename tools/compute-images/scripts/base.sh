...
if [ "$NVIDIA_GPU_SUPPORT" == "1" ]; then
  ...
  if [[ -f /etc/modules-load.d/nvidia.conf ]]; then
      mv /etc/modules-load.d/nvidia.conf /etc/modules-load.d/nvidia.avail
  fi
  install "$WORKDIR/usr-local-bin-detect-gpu.sh" /usr/local/bin/detect-gpu.sh
  install -d /etc/systemd/system/systemd-modules-load.service.d
  install -m 0644 \

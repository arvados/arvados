...
func (conn *Conn) generated_CredentialList(ctx context.Context, options arvados.ListOptions) (arvados.CredentialList, error) {
	var mtx sync.Mutex
	var merged arvados.CredentialList
	var needSort atomic.Value
	needSort.Store(false)
	err := conn.splitListRequest(ctx, options, func(ctx context.Context, _ string, backend arvados.API, options arvados.ListOptions) ([]string, error) {
		options.ForwardedFor = conn.cluster.ClusterID + "-" + options.ForwardedFor
		cl, err := backend.CredentialList(ctx, options)
		if err != nil {
			return nil, err
		}
		mtx.Lock()
	...
	if merged.Items == nil {
		...
		// (https://github.com/golang/go/issues/27589 might be
		// a better solution in the future)
		merged.Items = []arvados.Credential{}
	}
	return merged, err
}

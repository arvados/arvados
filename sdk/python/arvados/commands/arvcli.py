...
def _handle_resource_call(args, remaining_args, cmd_parser, api_client):
    ...
    if resource is not None:
        ...
        try:
            method_kwargs = vars(args.method_parameters)
        except AttributeError:
            method_kwargs = {}
        resource_obj = getattr(api_client, resource)()
        method_request = getattr(resource_obj, args.method)
        method_call = method_request(**method_kwargs)

        try:
            arv_object = method_call.execute()
        ...
        match args.format:
            ...
            case "yaml":
                ...
                yaml.dump(arv_object, sys.stdout)
            case "uuid":
                if (
                        arv_object.get("kind", "").endswith("List")
                        and arv_object.get("items")
                ):
                    for item in arv_object["items"]:
                        print(item["uuid"])
                else:
                    obj_uuid = arv_object.get("uuid")
                    if obj_uuid is None:
                        print(
                            "Error: response did not include a uuid:",
                            json.dumps(arv_object, indent=1),
                            sep="\n",
                            file=sys.stderr
                        )
                        sys.exit(1)

...
func newMetrics(reg *prometheus.Registry) *metrics {
	m := &metrics{
		...
		mDownloadBackendSpeed: prometheus.NewHistogramVec(prometheus.HistogramOpts{
			...
			Help:      "Apparent download speed from the backend (bytes per second) when serving file downloads, bucketed by transfer size range (see https://github.com/arvados/arvados/blob/main/doc/development/WebDAVPerformanceMetrics.md for explanation)",
			Buckets:   []float64{10_000, 1_000_000, 10_000_000, 100_000_000, 1_000_000_000, math.Inf(+1)},
		}, []string{"size_range"}),

...
class Workflow < ArvadosModel
  ...
  def validate_collection_uuid
    ...
    if c.properties["type"] != "workflow"
      errors.add :collection_uuid, "properties.type is not 'workflow'"
    end

    if c.properties["arv:workflowMain"].nil?
      errors.add :collection_uuid, "collection properties does not include arv:workflowMain"
    end
  end
  ...
  def link_with_collection
    ...
    Collection.find_by_uuid(collection_uuid).update_linked_workflows self
  end
